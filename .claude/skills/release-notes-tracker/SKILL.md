...
3. Створи `meta.md` з `last_processed` = версія, включно до якої виконано початкове заповнення (без `file_rotation.*` полів — вони додаються при першій ротации в Step 6.5)
