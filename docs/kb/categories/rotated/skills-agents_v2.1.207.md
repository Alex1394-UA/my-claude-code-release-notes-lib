...
| `/agents` | Список агентів | 1.0.60 |
| `/doctor` | Діагностика (повна діагностика з можливістю виправлення); `/checkup` — аліас | 1.0.51, 2.1.205 |
...
| Можливість | Опис | Версія |
|-----------|------|--------|
| Фонові агенти статус після відновлення fix | Виправлено фонові агенти що залишалися в статусі "failed" або "completed" у списку агентів після відновлення через `SendMessage` | 2.1.205 |
| Фонові задачі flip статусу fix | Виправлено фонові задачі що переключались з "needs input" назад на "working" у списку агентів коли turn агента не містив читабельного тексту | 2.1.205 |
| Verify skills перезапис fix | Виправлено project verify skills що перезаписувалися при кожній сесії замість лише при зміні задокументованої команди | 2.1.205 |

## Навички та Агенти (2.1.206)

| Можливість | Опис | Версія |
|-----------|------|--------|
| `/commit-push-pr` автоматичний дозвіл git push | `/commit-push-pr` тепер автоматично дозволяє `git push` до налаштованого push remote (`remote.pushDefault`, або єдиного remote коли лише один) на додачу до `origin` | 2.1.206 |
| Фонові агенти фонове оновлення | Фонові агенти тепер оновлюються до нової версії у фоні одразу після оновлення Claude Code, замість повільного stale-session upgrade при attach | 2.1.206 |
| Agents view статус на повну ширину | Agents view: стовпець статусу тепер використовує повну ширину термінала замість обрізання на 64 символах | 2.1.206 |
| Agents view Ctrl+X видалення сесії | Agents view: `Ctrl+X` тепер назавжди видаляє завершену сесію, сесії більше не рендеряться двічі; видалені background jobs залишаються видаленими | 2.1.206 |
| `/code-review` якість результатів | Покращено якість результатів `/code-review` на claude-opus-4-8 на всіх рівнях зусиль | 2.1.206 |
| Agents view keyboard input Windows fix [Win] | Виправлено ігнорування вводу з клавіатури у agents view коли setup prompt з'являвся перед bare `claude --resume` на Windows | 2.1.206 |

## Навички та Агенти (2.1.207)

| Можливість | Опис | Версія |
|-----------|------|--------|
| Agent teams crash loop fix | Виправлено crash loop в agent teams коли malformed teammate mailbox message викликав repeated errors щосекунди до ручного видалення mailbox file | 2.1.207 |
| Background session plan name fix | Виправлено background sessions що автоматично називались прийняттям плану але не показували це ім'я на рядку agent-view | 2.1.207 |
| Background worktree cold reopen fix | Виправлено background sessions що увійшли в git worktree та відновлювались порожніми після cold reopen з agent list | 2.1.207 |
| Deep research Fetch-phase agent labels fix | Виправлено Deep research runs що позначали кожного Fetch-phase agent як «unknown» — chips тепер показують source hostname | 2.1.207 |
| Agent view paste placeholder expand | Agent view: повторна вставка того ж тексту тепер розгортає згорнутий `[Pasted text #N]` placeholder замість додавання другого | 2.1.207 |
| Agent view blocked peek staleness | Agent view: blocked session peeks тепер починаються з питання та показують worded staleness clock (`waiting 3m`) замість того ж timestamp двічі | 2.1.207 |
