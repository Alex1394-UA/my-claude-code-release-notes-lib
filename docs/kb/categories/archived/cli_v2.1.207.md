...
| Можливість | Опис | Версія |
|-----------|------|--------|
| `--json-schema` невалідна схема fix | Виправлено `--json-schema` що мовчки видавав неструктурований вивід при невалідній схемі, та відхилення schem з ключовим словом `format` | 2.1.205 |
| `--max-turns` втрата повідомлення fix | Виправлено втрату повідомлення надісланого під час роботи Claude коли turn завершувався на ліміті `--max-turns` | 2.1.205 |
| Видалення worktree поза межами fix [Win] | Виправлено видалення worktree що видаляло файли поза межами worktree коли всередині існував NTFS junction або directory symlink | 2.1.205 |
| `claude attach` під час оновлення fix | Виправлено `claude attach` що видавав помилку коли фоновий агент перезапускався через оновлення — тепер очікує його повернення | 2.1.205 |
| Зв'язок сесії з PR через Bash fix | Виправлено зв'язок сесії з PR що не створювався для PR створеного через Bash з виводом понад 30K символів | 2.1.205 |
| `claude mcp add-from-claude-desktop` імена fix | Виправлено зависання при імені сервера з неприпустимими символами — невалідні імена тепер повідомляються, решта серверів імпортується | 2.1.205 |
| Agent view рендеринг заголовка fix | Виправлено agent view що рендерив на один рядок вище та обрізав заголовок коли список задач трохи виходив за межі екрану | 2.1.205 |
| Agent view посилання на PR | Agent view: сесії що редагують, мержать, коментують або пушать існуючий PR тепер показують посилання на нього в `claude agents` | 2.1.205 |
| Agent view кольоровий стан та заголовок | Agent view: рядки тепер показують кольорове слово стану та згенерований класифікатором заголовок замість сирих викликів інструментів; peek відкривається з повним статусом включаючи точний запит для заблокованих сесій | 2.1.205 |
| Auto-update потокове завантаження | Бінарні оновлення тепер завантажуються потоком на диск замість буферизації в пам'яті, зменшуючи пікове споживання пам'яті оновлювача на ~400 MB | 2.1.205 |
...
| Можливість | Опис | Версія |
|-----------|------|--------|
| `/usage-credits` amount validation fix | Виправлено `/usage-credits` amount inputs що мовчки обрізали malformed values до цифр; malformed amounts тепер відхиляються з помилкою, а суми понад $1,000 вимагають підтвердження вводом з клавіатури | 2.1.207 |
