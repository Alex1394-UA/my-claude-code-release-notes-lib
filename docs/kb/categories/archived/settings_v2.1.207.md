...
| "Dangerous rm operation" false-positive fix [Win] | Виправлено хибні "Dangerous rm operation" permission prompts in auto mode для багаторядкових bash команд з pipe та redirect | 2.1.120 |
...
| `NO_COLOR`/`FORCE_COLOR` in settings.json `env` fix | `NO_COLOR`/`FORCE_COLOR` в settings.json `env` більше не stripають Claude Code UI кольори — тепер застосуються лише до subprocesses | 2.1.143 |
...
| Glob patterns у deny rules | Підтримка glob-патернів у позиції назви інструменту в deny-правилах (`"*"` забороняє всі інструменти); allow-правила відхиляють non-MCP globs, невідомі назви в deny rules викликають попередження при старту | 2.1.166 |
...
| Ключ | Опис | Версія |
|------|------|--------|
| Organization default models [Managed] | Адміни встановлюють модель за замовчуванням для org in console; користувачі що не обрали власну модель бачать "Org default" або "Role default" в `/model` picker | 2.1.196 |
