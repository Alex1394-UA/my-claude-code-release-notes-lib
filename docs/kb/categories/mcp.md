...
## Налаштування (2.1.205)

| Можливість | Опис | Версія |
|-----------|------|--------|
| Зарезервовано імена MCP серверів | Імена "Claude Browser" та "Claude Preview" зарезервовано — користувацькі MCP сервери не можуть бути зареєстровані під цими іменами | 2.1.205 |

## Налаштування (2.1.206)

| Можливість | Опис | Версія |
|-----------|------|--------|
| `request_timeout_ms` ignored fix | Виправлено MCP сервери налаштовані через `--mcp-config` або `.mcp.json` що ігнорували per-server `request_timeout_ms`, через що довготривалі MCP tool calls переривались за 60s default timeout у fresh sessions | 2.1.206 |
| OAuth MCP re-auth fix | Виправлено OAuth MCP сервери що вимагали повторну автентифікацію після одного невдалого token refresh | 2.1.206 |
