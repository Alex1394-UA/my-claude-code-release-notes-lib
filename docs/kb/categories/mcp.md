# MCP (Model Context Protocol)

> Архів (notes до 2.1.207): [archived/mcp_v2.1.207.md](archived/mcp_v2.1.207.md)
> Тут — notes починаючи з версії 2.1.208

## Налаштування (2.1.208)

| Можливість | Опис | Версія |
|-----------|------|--------|
| MCP stdio stderr memory leak fix | Виправлено витік пам'яті: MCP stdio сервер накопичував до 64 MB stderr на сервер у довгих сесіях | 2.1.208 |
| Tool-pool caching fix | Кешування tool-pool зменшує CPU-навантаження за tool-call у print/SDK сесіях з великим числом MCP інструментів | 2.1.208 |
| MCP empty URL "not configured" | MCP сервери з порожнім URL показуються як "not configured" у `/mcp` замість помилки конфігу | 2.1.208 |

## Налаштування (2.1.210)

| Можливість | Опис | Версія |
|-----------|------|--------|
| Plugin MCP re-sync teardown fix | Plugin-provided MCP сервери більше не знищуються під час повторної синхронізації MCP серверів посеред сесії | 2.1.210 |

## Налаштування (2.1.211)

| Можливість | Опис | Версія |
|-----------|------|--------|
| Plugin MCP servers reconnect after idle web wake fix | Виправлено перепідключення plugin MCP-серверів після пробудження простоюючої веб-сесії — раніше MCP-виклики зазнавали невдачі до наступного повідомлення | 2.1.211 |

## Налаштування (2.1.212)

| Можливість | Опис | Версія |
|-----------|------|--------|
| MCP auto-background long calls | Виклики MCP-інструментів, що тривають понад 2 хвилини, тепер автоматично переходять у фон, щоб сесія лишалась придатною; поріг налаштовується або вимикається через `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS` | 2.1.212 |

## Налаштування (2.1.214)

| Можливість | Опис | Версія |
|-----------|------|--------|
| MCP transient errors refresh clearing fix | Виправлено очищення slash-команд та resources сервера при transient помилках MCP під час prompts/resources refresh | 2.1.214 |
